<?php /*a:4:{s:61:"/www/wwwroot/guang.ailiutang.cn/app/view/binding/toutiao.html";i:1762426633;s:56:"/www/wwwroot/guang.ailiutang.cn/app/view/public/css.html";i:1762426633;s:55:"/www/wwwroot/guang.ailiutang.cn/app/view/public/js.html";i:1762426633;s:62:"/www/wwwroot/guang.ailiutang.cn/app/view/public/copyright.html";i:1762426633;}*/ ?>
<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
					...
					<div class="layui-form form-label-w8" lay-filter="" style="margin-top:20px">
						...
						<div class="layui-form-item">
							...
							<div class="layui-input-inline">
								<input type="radio" name="info[toutiaopay]" title="开启" value="1" <?php if($info['toutiaopay']==1): ?>checked<?php endif; ?>/>
								<input type="radio" name="info[toutiaopay]" title="关闭" value="0" <?php if($info['toutiaopay']==0): ?>checked<?php endif; ?>/>
							</div>
						</div>
						<div class="layui-form-item">
							<label class="layui-form-label">Token(令牌)：</label>
							<div class="layui-input-inline" style="width:280px">
								<input type="text" name="info[pay_token]" value="<?php echo $info['pay_token']; ?>" class="layui-input">
							</div>
						</div>
						<div class="layui-form-item">
							<label class="layui-form-label">商户号：</label>
							<div class="layui-input-inline" style="width:280px">
								<input type="text" name="info[pay_mchid]" value="<?php echo $info['pay_mchid']; ?>" class="layui-input">
							</div>
						</div>
						<div class="layui-form-item">
							<label class="layui-form-label">SALT：</label>
							<div class="layui-input-inline" style="width:350px">
								<input type="text" name="info[pay_salt]" value="<?php echo $info['pay_salt']; ?>" class="layui-input">
								<?php if($info['pay_salt']): ?><div style="position: absolute;left: 0;top: 0;right: 0;bottom: 0; background: #fff;padding: .35rem .7rem;border: 1px solid rgba(0, 0, 0, .15);border-radius: .15rem;color: #636c72;" onclick="$(this).hide()">已隐藏内容，点击查看或编辑</div><?php endif; ?>
							</div>
						</div>
						<div class="layui-form-item">
							<label class="layui-form-label">服务器地址：</label>
							<div class="layui-form-mid"><?php echo PRE_URL2; ?></div>
						</div>
						<div class="layui-form-item">
...
<script>
	...
	function downloadxcx(){
		layer.confirm('下载代码包用开发者工具进行上传代码，确定要下载吗?',function(index){
			...
			layui.form.on('submit(submit_downloadxcx)', function(obj){
				var field = obj.field;
				var index = layer.load();
				$.post("<?php echo url('downloadtoutiaoxcx'); ?>",field,function(data){
					layer.close(index);
	...
	layui.form.on('submit(submit_reg)', function(obj){
		...
		$.post('',field,function(data){
			dialog(data.msg,data.status,'<?php echo url('toutiao'); ?>');
		})
	})
	layui.upload.render({
