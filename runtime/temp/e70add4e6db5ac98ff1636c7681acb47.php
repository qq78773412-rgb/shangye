<?php /*a:3:{s:59:"/www/wwwroot/guang.ailiutang.cn/app/view/binding/index.html";i:1762426633;s:56:"/www/wwwroot/guang.ailiutang.cn/app/view/public/css.html";i:1762426633;s:55:"/www/wwwroot/guang.ailiutang.cn/app/view/public/js.html";i:1762426633;}*/ ?>
<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
  ...
  <div class="layui-fluid">
    <div class="layui-row layui-col-space15">
		...
		<div class="layui-card layui-col-md12">
			...
			<div class="layui-card-body" pad15>
				<div class="layui-col-md12" style="font-size:16px;padding:20px 0">
					<?php if($mpappinfo && $mpappinfo['appid']): ?>
					<div class="layui-col-md6" style="display:flex;align-items:center">
						<div style="display:flex;">
							<img src="<?php echo (isset($mpappinfo['headimg']) && ($mpappinfo['headimg'] !== '')?$mpappinfo['headimg']:'/static/img/wxtx.png'); ?>" style="width:40px;height:40px;border-radius:50%"/>
							<div style="margin-left:10px;margin-right:10px;display:flex;flex-direction:column">
								<span><?php echo (isset($mpappinfo['nickname']) && ($mpappinfo['nickname'] !== '')?$mpappinfo['nickname']:'暂无昵称'); ?></span>
								<span style="font-size:10px;color:#73d78e;line-height:12px;">
									<?php if($mpappinfo['level']==2 || $mpappinfo['level']==4): ?>服务号<?php else: ?>订阅号<?php endif; ?>&nbsp;
									<?php if($mpappinfo['level']==3 || $mpappinfo['level']==4): ?><span style="color:#53d78e;"><i class="fa fa-check-circle"></i> 已认证</span><?php else: ?><span style="color:#f85;">未认证</span><?php endif; ?>
								</span>
							</div>
						</div>
						<?php if($mpappinfo['authtype']==1): ?>
						<div style="display:flex;align-items:center">
							...
							<div style="display:flex;flex-direction:column;font-size:13px">
								<span style="color:#999;margin-right:20px">AppID： <?php echo $mpappinfo['appid']; ?></span>
								<?php if($mpappinfo['open_appid'] && $mpappinfo['open_appid']!='-1'): ?><span style="color:#999;margin-right:20px">开放平台APPID： <?php echo !empty($mpappinfo['open_appid']) ? $mpappinfo['open_appid'] :  '未绑定'; ?></span><?php endif; ?>
							</div>
						</div>
						<?php else: ?>
						<div>
							<span style="color:#53d78e;margin-right:30px"><i class="fa fa-check-circle"></i> 已绑定</span>
							<span style="color:#999;margin-right:20px;font-size:13px">AppID： <?php echo $mpappinfo['appid']; ?></span>
						</div>
						<?php endif; ?>
					</div>
					<div class="layui-col-md6" style="text-align:right">
						<button onclick="showqrcode('<?php echo $mpappinfo['qrcode']; ?>')" class="layui-btn layui-btn-primary">查看二维码</button>
		...
		<div class="layui-card layui-col-md12">
			...
			<div class="layui-card-body" pad15>
				<div class="layui-col-md12" style="font-size:16px;padding:20px 0">
				<?php if($wxappinfo && $wxappinfo['appid']): ?>
					<div class="layui-col-md6" style="display:flex;align-items:center">
						<div style="display:flex;">
							<img src="<?php echo (isset($wxappinfo['headimg']) && ($wxappinfo['headimg'] !== '')?$wxappinfo['headimg']:'/static/img/wxtx.png'); ?>" style="width:40px;height:40px;border-radius:50%"/>
							<div style="margin-left:10px;margin-right:10px;display:flex;flex-direction:column">
								<span><?php echo (isset($wxappinfo['nickname']) && ($wxappinfo['nickname'] !== '')?$wxappinfo['nickname']:'暂无昵称'); ?></span>
								<span style="font-size:10px;color:#73d78e;line-height:12px;">
									小程序&nbsp;
									<?php if($wxappinfo['authtype']==1): if($wxappinfo['level']==1): ?><span style="color:#53d78e;"><i class="fa fa-check-circle"></i> 已认证</span><?php else: ?><span style="color:#f85;">未认证</span><?php endif; ?>
									<?php endif; ?>
								</span>
							</div>
						</div>
						<?php if($wxappinfo['authtype']==1): ?>
						<div style="display:flex;align-items:center">
							...
							<div style="display:flex;flex-direction:column;font-size:13px">
								<span style="color:#999;margin-right:20px">AppID： <?php echo $wxappinfo['appid']; ?></span>
								<?php if($wxappinfo['open_appid'] && $wxappinfo['open_appid']!='-1'): ?><span style="color:#999;margin-right:20px">开放平台APPID： <?php echo !empty($wxappinfo['open_appid']) ? $wxappinfo['open_appid'] :  '未绑定'; ?></span><?php endif; ?>
							</div>
						</div>
						<?php else: ?>
						<div>
							<span style="color:#53d78e;margin-right:30px"><i class="fa fa-check-circle"></i> 已手动绑定</span>
							<span style="color:#999;margin-right:20px;font-size:13px">AppID： <?php echo $wxappinfo['appid']; ?></span>
						</div>
						<?php endif; ?>
					</div>
					<div class="layui-col-md6" style="text-align:right">
						...
						<p>
							<button onclick="<?php if($wxappinfo['authtype']==1): ?>commit()<?php else: ?>showsd()<?php endif; ?>" class="layui-btn layui-btn-normal">上传代码</button>
							<?php if($wxappinfo['createtype']==1 || $wxappinfo['createtype']==2): ?><button onclick="openmax('<?php echo url('Wxset/index'); ?>&isopen=1')" class="layui-btn layui-btn-primary">设置</button><?php endif; ?>
							<a href="javascript:void(0)" onclick="tobangding('wx')" class="layui-btn layui-btn-primary">重新绑定</a>
							<?php if($componentinfo['status']==1): ?><a href="javascript:void(0)" onclick="regxcx()" class="layui-btn layui-btn-primary">重新创建</a><?php endif; if($wxalogList): ?><button onclick="openmax('<?php echo url('Wxset/yinsi'); ?>&isopen=1')" class="layui-btn layui-btn-primary">隐私设置</button><?php endif; if($wxappinfo['authtype']==1): ?>
							<button onclick="openmax('<?php echo url('Wxset/privacyInterfaceList'); ?>&isopen=1')" class="layui-btn layui-btn-primary">接口申请</button>
							<button onclick="openmax('<?php echo url('Wxset/icp'); ?>&isopen=1')" class="layui-btn layui-btn-primary">备案</button>
							<?php endif; ?>
						</p>
						<?php if($wxappinfo['authtype']==1): ?>
						<p style="margin-top: 10px">
							<button onclick="openmax('<?php echo url('Wxset/serverDomain'); ?>&isopen=1')"  class="layui-btn layui-btn-primary">服务器域名</button>
							<button onclick="openmax('<?php echo url('Wxset/jumpDomain'); ?>&isopen=1')"  class="layui-btn layui-btn-primary">业务域名</button>
							<?php if(getcustom('test')): ?>
				...
				<?php foreach($wxreglog as $k=>$wxreg): ?>
				<div class="layui-col-md12" style="padding:10px 0 10px 20px;font-size:14px;<?php if($k+1<count($wxreglog)): ?>border-bottom:1px solid #eee<?php endif; ?>">
					<div>企业名称：<?php echo $wxreg['name']; ?> </div>
					<div><?php if($wxreg['code_type']==1): ?>统一社会信用代码<?php elseif($wxreg['code_type']==2): ?>组织机构代码<?php elseif($wxreg['code_type']==3): ?>营业执照注册号<?php endif; ?>：<?php echo $wxreg['code']; ?> </div>
					<div>企业法人：<?php echo $wxreg['legal_persona_name']; ?> 微信号：<?php echo $wxreg['legal_persona_wechat']; ?></div>
					<div>微信号：<?php echo $wxreg['legal_persona_wechat']; ?></div>
					<div>
						状态：<?php if($wxreg['status']==0): ?><span style="color:#f85">申请中</span> 注意查看微信消息通知并点击完成验证 <?php elseif($wxreg['status']==1): ?><span style="color:#55f">待注册</span><?php elseif($wxreg['status']==2): ?><span style="color:red">已驳回 <?php echo $wxreg['reason']; ?></span><?php endif; if($wxreg['status']!=2): ?>&nbsp;&nbsp;<span style="color:#aaa;margin-right:15px;cursor:pointer" onclick="get_regstatus('<?php echo $wxreg['id']; ?>')"><i class="fa fa-refresh"></i></span><?php endif; ?>
					</div>
				</div>
				<?php endforeach; ?>
				<?php endif; if($wxappinfo && $wxappinfo['authtype']==1 && !$wxalogList): ?>
				...
				<?php endif; foreach($wxalogList as $k=>$wxalog): ?>
				<div class="layui-col-md12">
				<div class="layui-col-md6" style="padding:10px 0 10px 20px;font-size:14px;border-top:1px solid #eee">
					<div style="font-weight:bold">上传时间：<?php echo $wxalog['createtime']; ?> </div>
					<div>版本号：<?php echo $wxalog['user_version']; ?> </div>
					<div>版本描述：<?php echo $wxalog['user_desc']; ?> </div>
					<?php if($wxalog['category']): ?><div>服务类目：<?php echo $wxalog['category']; ?> </div><?php endif; ?>
					<div>状态：<?php if($wxalog['status']==0): ?>未提交<?php elseif($wxalog['status']==1): ?><span style="color:#f85">审核中</span>
						<span style="color:#aaa;margin-right:15px;cursor:pointer" onclick="get_auditstatus('<?php echo $wxalog['auditid']; ?>')"><i class="fa fa-refresh"></i></span><?php elseif($wxalog['status']==2): ?><span style="color:#55f">待发布</span><?php elseif($wxalog['status']==3): ?><span style="color:green">已发布</span><?php elseif($wxalog['status']==4): ?><span style="color:red">已驳回</span><?php endif; ?></div>
				</div>
				<div class="layui-col-md6" style="text-align:right;border-top:1px solid #eee;padding:30px 0;">
					<?php if($k==0 && $wxalog['status']==0): ?><button class="layui-btn layui-btn-primary" onclick="openchooseHy()">提交审核</button><?php endif; if($k==0): ?><button class="layui-btn layui-btn-primary" onclick="showqrcode('<?php echo url('gettyqrcode'); ?>')">体验二维码</button><?php endif; if($wxalog['status']==4): ?><button class="layui-btn layui-btn-primary" onclick="audit_reason(<?php echo $wxalog['id']; ?>)">失败原因</button><?php endif; if($wxalog['status']==2): ?><button class="layui-btn layui-btn-primary" onclick="fabu()">发布</button><?php endif; if($wxalog['status']==3): ?><button class="layui-btn layui-btn-primary" onclick="showqrcode('<?php echo $wxappinfo['qrcode']; ?>')">查看二维码</button><?php endif; if($wxalog['status']==1): ?><button class="layui-btn layui-btn-primary" onclick="chexiao()">撤销审核</button><?php endif; if($k==0): ?><button onclick="addtester()" class="layui-btn layui-btn-primary">添加体验者</button><?php endif; ?>
					<div style="display:none" id="audit_reason_<?php echo $wxalog['id']; ?>"><?php echo $wxalog['audit_reason']; ?></div>
				</div>
				</div>
				<?php endforeach; ?>
			</div>
	...
	<div id="chooseHyModel" style="width:600px;display:none;margin-top:30px">
		<div class="layui-form form-label-w6" lay-filter="">
			<input type="hidden" name="template_id" value="<?php echo $tpldata['template_id']; ?>"/>
...
<script>
	...
	<script>
		var mode = "<?php echo (isset($mode) && ($mode !== '')?$mode:0); ?>"
	function get_auditstatus(auditid){
		var index = layer.load();
		$.get("<?php echo url('get_auditstatus'); ?>",{auditid:auditid},function(data){
			layer.close(index);
	...
	function get_regstatus(id){
		var index = layer.load();
		$.get("<?php echo url('get_regstatus'); ?>",{id:id},function(data){
			layer.close(index);
			dialog(data.msg,data.status,location.href)
		});
	}
	function refreshauthinfo(apptype){
		var index = layer.load();
		$.post("<?php echo url('refreshauthinfo'); ?>",{apptype:apptype},function(data){
			layer.close(index);
			dialog(data.msg,data.status,location.href)
		});
	}
	...
	function commit(){
		...
		html+='			3、获取当前的地理位置、速度（wx.getLocation接口）用于配送端实时展示配送员的地理位置及展示门店和商户的距离 <a href="javascript:void(0)" onclick="openmax(\'<?php echo url('Wxset/privacyInterfaceList'); ?>&isopen=1\')">点击前往申请</a><br>';
		...
		html+='	<div class="layui-form-item">';
		html+='		<label class="layui-form-label"><?php echo t('会员'); ?>中心导航栏样式：</label>';
		...
		html+='		</div>';
		html+='	</div>';
		<?php if(getcustom('business_index_navigation_style')): ?>
		html+='<div class="layui-form-item">';
		...
		html+='			<input type="text" name="version" class="layui-input" value="<?php echo $version; ?>">';
		html+='		</div>';
		html+='	</div>';
		html+='	<div class="layui-form-item">';
		...
		layui.form.on('submit(submit_commit)', function(obj){
			var field = obj.field;
			var index = layer.load();
			$.post("<?php echo url('commit'); ?>",field,function(data){
				layer.close(index);
	...
	function xcxqrcode(){
		$.post("<?php echo url('getqrcode'); ?>",{},function(data){
			if(data.status==0){
	...
	function addtester(){
		...
		layui.form.on('submit(submit_bind_tester)', function(obj){
			...
			console.log(field);
			var index= layer.load();
			$.get("<?php echo url('bind_tester'); ?>",field,function(data){
				layer.close(index);
				dialog(data.msg,data.status,data.url);
			})
		})
	...
	function fabu(){
		layer.confirm('确定要发布上线吗?', function(index){
			layer.close(index);
			var index = layer.load();
			$.post("<?php echo url('fabu'); ?>",{},function(data){
				layer.close(index);
				dialog(data.msg,data.status,data.url)
			});
		});
	...
	function chexiao(){
		layer.confirm('每天最多撤销一次，一个月不超过10次，确定要撤销审核吗?', function(index){
			layer.close(index);
			var index = layer.load();
			$.post("<?php echo url('undocodeaudit'); ?>",{},function(data){
				layer.close(index);
				dialog(data.msg,data.status,data.url)
			});
		});
	...
	function openchooseHy(){
		var index = layer.load();
		$.post("<?php echo url('getHy'); ?>",{},function(data){
			layer.close(index);
  ...
  layui.form.on('submit(submit_audit)', function(obj){
		...
		var index= layer.load();
		$.post("<?php echo url('submit_audit'); ?>",{hy:field.hyselect,tag:field.tag,autofb:field.autofb,order_path:field.order_path},function(data){
			layer.close(index);
			dialog(data.msg,data.status,data.url);
		})
	})
	...
	function showsd(){
		...
		html+='		<div style="margin:0 20px;width:150px;height:70px;line-height:70px;background:#fff;border:1px solid #e7e7eb;border-radius:5px;text-align:center;position:relative;cursor:pointer" onclick="openmax(\'<?php echo url('xianchuan'); ?>/isopen/1\')">扫码上传</div>';
		...
		layer.confirm('手动上传需要下载代码包，用微信开发者工具进行上传代码，确定要下载吗?',function(index){
			...
			html+=`			<label class='radio-label-class'>
										...
										<div class="layui-popover layui-default-link layui-inline">
											...
											<div class="layui-popover-div layui-popover-custom">
												<img src="/static/admin/img/dianda_mp_business.png" />
											</div>
										</div>
									</label>`;
			html+=`			<label class='radio-label-class'>
										...
										<div class="layui-popover layui-default-link layui-inline">
											示例
											<div class="layui-popover-div layui-popover-custom">
												<img src="/static/admin/img/dianda_mp_business2.png" />
											</div>
										</div>
									</label>`;
			html+=`			<label class='radio-label-class'>
										...
										<div class="layui-popover layui-default-link layui-inline">
											...
											<div class="layui-popover-div layui-popover-custom" style="left:-230px">
												<img src="/static/admin/img/dianda_mp_binding3.png" />
											</div>
										</div>
									</label>`;
			<?php if(getcustom('show_location')): ?>
				if(mode==2){
					html+=`			<label class='radio-label-class'>
												...
												<div class="layui-popover layui-default-link layui-inline">
													示例
													<div class="layui-popover-div layui-popover-custom">
														<img src="/static/admin/img/dianda_mp_binding4.png" />
													</div>
												</div>
											</label>`;
					html+=`			<label class='radio-label-class'>
													...
													<div class="layui-popover layui-default-link layui-inline">
														示例
														<div class="layui-popover-div layui-popover-custom">
															<img src="/static/admin/img/dianda_mp_binding5.png" />
														</div>
													</div>
											</label>`;
					html+=`			<label class='radio-label-class'>
												...
												<div class="layui-popover layui-default-link layui-inline">
													示例
													<div class="layui-popover-div layui-popover-custom">
														<img src="/static/admin/img/dianda_mp_binding6.png" />
													</div>
												</div>
											</label>`;
				}else if(mode==3) {
					html+=`			<label class='radio-label-class'>
												...
												<div class="layui-popover layui-default-link layui-inline">
													示例
													<div class="layui-popover-div layui-popover-custom">
														<img src="/static/admin/img/dianda_mp_binding7.png" />
													</div>
												</div>
											</label>`;
					html+=`			<label class='radio-label-class'>
												...
												<div class="layui-popover layui-default-link layui-inline">
													示例
													<div class="layui-popover-div layui-popover-custom">
														<img src="/static/admin/img/dianda_mp_binding9.png" />
													</div>
												</div>
											</label>`;
					html+=`			<label class='radio-label-class'>
												...
												<div class="layui-popover layui-default-link layui-inline">
													示例
													<div class="layui-popover-div layui-popover-custom">
														<img src="/static/admin/img/dianda_mp_bindingx.png" />
													</div>
												</div>
											</label>`;
				}
			...
			html+=`			<label  class='radio-label-class'>
										...
										<div class="layui-popover layui-default-link layui-inline">
											示例
											<div class="layui-popover-div layui-popover-custom">
												<img src="/static/admin/img/dianda_mp_main22.png" />
											</div>
										</div>
									</label>`;
			html+=`			<label  class='radio-label-class'>
										...
										<div class="layui-popover layui-default-link layui-inline">
											示例
											<div class="layui-popover-div layui-popover-custom">
												<img src="/static/admin/img/dianda_mp_main1.png" />
											</div>
										</div>
									</label>`;
			html+='		</div>';
			...
			html+='<div class="layui-form-item">' +
					...
					`<label class='radio-label-class'>
						...
						<div class="layui-popover layui-default-link layui-inline">
							示例
							<div class="layui-popover-div layui-popover-custom">
								<img src="/static/admin/img/dianda_mp_main2.png" />
							</div>
						</div>
					</label>` +
					`<label class='radio-label-class'>
			...
			layui.form.on('submit(submit_downloadxcx)', function(obj){
				var field = obj.field;
				var index = layer.load();
				$.post("<?php echo url('downloadxcx'); ?>",field,function(data){
					layer.close(index);
					dialog(data.msg,data.status,data.url)
				});
			})
	...
	function tobangding(type){
		<?php if($componentinfo['status']==1 && PRE_URL==PRE_URL2): ?>
		var html = '';
		html+='<div style="margin:auto auto;text-align:center">';
		html+='	<div style="display:flex;justify-content:center;margin-top:40px;color:#333;font-size:20px;">';
		html+='		<a style="margin:0 20px;width:150px;height:70px;line-height:70px;background:#fff;border:1px solid #e7e7eb;border-radius:5px;text-align:center;position:relative;cursor:pointer;color: #333;" href="<?php echo url('sqbangding'); ?>/type/'+type+'" target="_blank">授权绑定</a>';
		html+='		<div style="margin:0 20px;width:150px;height:70px;line-height:70px;background:#fff;border:1px solid #e7e7eb;border-radius:5px;text-align:center;position:relative;cursor:pointer" onclick="openmax(\'<?php echo url('sdbangding'); ?>/type/'+type+'/isopen/1\')">手动绑定</div>';
		html+='	</div>'
		html+='</div>'
		layer.open({type:1,area:['600px','220px'],content:html,title:'请选择绑定方式',shadeClose:true})
		<?php else: ?>
		openmax("<?php echo url('sdbangding'); ?>/type/"+type+"/isopen/1");
		<?php endif; ?>
	}
	...
	function bindinopen(){
		layer.confirm('确定要开启粉丝账号数据互通吗?',function(index){
			layer.close(index);
			var index= layer.load();
			$.post("<?php echo url('bindinopen'); ?>",{},function(data){
				layer.close(index);
				dialog(data.msg,data.status,data.url);
			})
		})
	...
	function regxcx(){
		...
		<?php if($componentinfo['status']==1 && $webinfo['fastregxcx']!=2): ?>
	...
	function fastregxcx(){
		var index= layer.load();
		$.post("<?php echo url('fastregxcx'); ?>",{},function(data){
			layer.close(index);
			if(data.status==1){
	...
	function showregdialog(){
		...
		layui.form.on('submit(submit_reg)', function(obj){
			...
			console.log(field);
			var index= layer.load();
			$.post("<?php echo url('submit_reg'); ?>",field,function(data){
				layer.close(index);
				dialog(data.msg,data.status,data.url);
			})
		})
