<?php /*a:4:{s:61:"/www/wwwroot/guang.ailiutang.cn/app/view/wifiprint/index.html";i:1762426633;s:56:"/www/wwwroot/guang.ailiutang.cn/app/view/public/css.html";i:1762426633;s:55:"/www/wwwroot/guang.ailiutang.cn/app/view/public/js.html";i:1762426633;s:62:"/www/wwwroot/guang.ailiutang.cn/app/view/public/copyright.html";i:1762426633;}*/ ?>
<!DOCTYPE html> 
<html>
<head>
  <meta charset="utf-8">
  ...
  <div class="layui-fluid">
    <div class="layui-row layui-col-space15">
        <div class="layui-card layui-col-md12">
          <div class="layui-card-header">小票打印机</div>
          <div class="layui-card-body" pad15>
            <blockquote class="layui-elem-quote">小票打印机用于用户下单成功后自动打印小票+语音播报，目前支持易联云和飞鹅的小票打印机，请先购买小票打印机后进行配置，<a href="http://www.diandawl.cn/index/w/yilianyun" target="_blank">点击进入易联云购买地址</a>，<a href="http://www.diandawl.cn/index/w/feie" target="_blank">点击进入飞鹅购买地址</a></blockquote>
            <div class="layui-col-md7" style="padding-bottom:10px">
              <a class="layui-btn layuiadmin-btn-list" href="javascript:void(0)" onclick="openmax('<?php echo url('edit'); ?>')">添加易联云打印机</a>
              <a class="layui-btn layuiadmin-btn-list" href="javascript:void(0)" onclick="openmax('<?php echo url('edit2'); ?>')">添加飞鹅打印机</a>
            ...
            <div class="layui-form layui-form-search layui-col-md5">
...
<script>
  ...
  var tableIns = table.render({
    ...
    ,cols: [[ //表头
      ...
      {field: 'type', title: '类型',templet:function(d){
        if(d.type==0) return '易联云';
        if(d.type==1) return '飞鹅';
        if(d.type==2) return '映美云';
        if(d.type==3) return '大趋';
        if(d.type==4) return '芯烨';
        return '';
      }},
      {field: 'name', title: '名称'},
      {field: 'title', title: '标题'},
      {field: 'machine_code', title: '终端号'},
      //{field: 'msign', title: '终端密钥'},
      {field: 'createtime', title: '添加时间',templet:function(d){ return date('Y-m-d H:i',d.createtime)}},
      ...
      {field: 'operation', title: '操作',templet:function(d){
        ...
        if(d.type==1){
          html += '<button class="table-btn"  onclick="openmax(\'<?php echo url('edit2'); ?>/id/'+d.id+'\')">编辑</button>';
        }else if(d.type==2){
            html += '<button class="table-btn"  onclick="openmax(\'<?php echo url('edit3'); ?>/id/'+d.id+'\')">编辑</button>';
        }else if(d.type==3){
            html += '<button class="table-btn"  onclick="openmax(\'<?php echo url('edit4'); ?>/id/'+d.id+'\')">编辑</button>';
        }else if(d.type==4){
            html += '<button class="table-btn"  onclick="openmax(\'<?php echo url('edit5'); ?>/id/'+d.id+'\')">编辑</button>';
        }
        html += '<button class="table-btn" onclick="datadel('+d.id+')">删除</button>';
        html += '<button class="table-btn" onclick="printtest('+d.id+')">打印测试</button>';
        return html;
      }}
    ]]
  });
  ...
  function printtest(id){
    var index = layer.load();
    $.post("<?php echo url('printtest'); ?>",{id:id},function(data){
      layer.close(index);
