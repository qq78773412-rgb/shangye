				...
				<view class="order-box" @tap.stop="goto" :data-url="'orderdetail?id=' + item.id">
					...
					<view class="op">
						...
						<block v-if="item.status==1" >
							...
							<view v-if="item.totalprice>0 && (item.refund_status==0 || item.refund_status==3)" class="btn2" @tap.stop="goto" :data-url="'/pagesB/yueke/refund?orderid=' + item.id">申请退款</view>
						</block>
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      var st = that.st;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiYueke/orderlist', {st: st,pagenum: pagenum,keyword:that.keyword}, function (res) {
				that.loading = false;
