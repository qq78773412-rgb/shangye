...
<view class="container">
	<block v-if="isload">
		<dd-tab :itemdata="['全部','未核销','已核销']" :itemst="['all','1','2']" :st="st" :isfixed="true" @changetab="changetab"></dd-tab>
			...
			<block v-for="(item, index) in datalist" :key="index">
				<view class="order-box" @tap.stop="goto" :data-url="'orderdetail?id=' + item.id">
					<view class="content" style="border-bottom:none">
						<view @tap.stop="goto" :data-url="'orderdetail?id=' + item.id">
						...
						<view class="detail">
							...
							<view class="t4 flex">
								<view>
									<view class="t3"><text class="x1 flex1">{{item.price==0 || item.price==null?'免费':'支付金额：￥' + item.price}}</text></view>
									<view class="t2"><text>已邀请{{item.yqnum}}人</text></view>
								</view>
								<view v-if="item.status ==1" class="btn2">待核销</view>
								<view v-if="item.status ==2" class="btn2">已核销</view>
							</view>
						</view>
					</view>		
				</view>
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      var st = that.st;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiHbtkActivity/orderlist', {keyword: keyword,st: st,pagenum: pagenum}, function (res) {
			that.loading = false;
...
.order-box .content .detail .t2{height: 46rpx;line-height: 46rpx;color: #999;overflow: hidden;font-size: 22rpx; margin-top: 20rpx;}
...
.order-box .content .detail .btn2{ margin-top: 70rpx;}
