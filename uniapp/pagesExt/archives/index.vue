...
<view class="container">
	<block v-if="isload">
		...
		<view class="content" id="datalist">
			<view class="item" @tap="goto" :data-url="'detail?id=' + item.id" v-for="(item, index) in datalist" :key="index">
				<view class="f1">
  ...
  methods: {
    ...
    getdata: function (loadmore) {
      ...
      var st = that.st;
			...
			this.nomore = false;
			this.loading = true;
      app.post('ApiMemberArchives/index', {keyword:that.keyword,st: st,pagenum: pagenum}, function (res) {
				that.loading = false;
        var data = res.data;
        if (pagenum == 1) {
					that.datalist = data;
...
.content .item .f1 .t1{color:#000000;font-size:34rpx;word-break:break-all;overflow:hidden;text-overflow:ellipsis;}
