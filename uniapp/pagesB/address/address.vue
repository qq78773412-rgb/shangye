	...
	<block v-if="isload">
		...
		<view v-for="(item, index) in datalist" :key="index" class="content" @tap.stop="setdefault" :data-id="item.id">
			<view class="f1">
				...
				<image class="t3" :src="pre_url+'/static/img/edit.png'" @tap.stop="goto" :data-url="'addressadd?id=' + item.id + '&type=' + (item.latitude>0 ? '1' : '0') + '&source=' + source">
			</view>
		...
		<view class="btn-add" :class="menuindex>-1?'tabbarbot':'notabbarbot3'" :style="'background:linear-gradient(90deg,'+t('color1')+' 0%,rgba('+t('color1rgb')+',0.8) 100%)'" @tap="goto" :data-url="'addressadd?type=' + type + '&source=' + source"><image :src="pre_url+'/static/img/add.png'" style="width:28rpx;height:28rpx;margin-right:6rpx"/>添加地址</view>
	</block>
...
export default {
  ...
  onLoad: function (opt) {
		...
		this.source = this.opt.source || '';//来源 从什么类型商品下单过来如shop商城商品、product_thali套餐商品
		this.getdata();
  },
		...
		getdata:function(){
			...
			app.get('ApiAddress/address', {type: that.opt.type,keyword:that.keyword}, function (res) {
				...
				if (datalist.length == 0 && that.keyword == ''){
					uni.redirectTo({
						url: 'addressadd?type=' + (that.opt.type || 0) + '&source=' + (that.opt.source || '')
					});
