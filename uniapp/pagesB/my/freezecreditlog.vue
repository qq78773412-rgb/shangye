...
<view class="container">
	<block v-if="isload">
		<view class="myscore" :style="{background:t('color1')}">
			<view class="f1">我的{{t('冻结账户')}}</view>
			<view class="f2">{{freeze_credit}}<view v-if="canExchange==1" class="btn-mini" @tap="goto" data-url="/pagesB/my/freezeexchange">兑换</view></view>
		</view>
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      var st = that.st;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiQueueFree/freezeCreditLog', {st: st,pagenum: pagenum}, function (res) {
				that.loading = false;
        var data = res.data;
        if (pagenum == 1) {
					uni.setNavigationBarTitle({
						title: that.t('冻结账户') + '明细'
					});
					that.freeze_credit = res.freeze_credit;
					that.canExchange = res.can_exchange;
					that.datalist = data;
          if (data.length == 0) {
            that.nodata = true;
          }
...
<style>
.myscore{width:94%;margin:20rpx 3%;border-radius: 10rpx 10rpx 10rpx 10rpx;position:relative;display:flex;flex-direction:column;padding:70rpx 0}
