	...
	<block v-if="isload">
		...
		<view class="orderinfo">
			...
			<view class="item">
				...
				<text class="t2">{{detail.apply_time}}</text>
			</view>
			<view class="item" v-if="detail.refund_status>0">
				...
				<text class="t2 red" v-if="detail.refund_status==-1">已驳回</text>
			</view>
		...
		<view class="orderinfo">
			...
			<view class="item" v-if="order.status>1 && order.daodian_time">
				<text class="t1">入住时间</text>
		...
		<view class="orderinfo">
			...
			<view class="item">
				...
				<text class="t2" v-if="order.status==2">待入住</text>
				<text class="t2" v-if="order.status==3">已入住</text>
		...
		<view class="bottom">
			<block v-if="detail.refund_status==1">
				...
				<view  class="btn2" @tap="refundpass" :data-id="detail.id">退款通过</view>
			</block>
...
export default {
  ...
  methods: {
		getdata: function () {
			var that = this;
			that.loading = true;
			app.get('ApiAdminHotelOrder/yajinRefundOrderDetail', {id: that.opt.id}, function (res) {
				that.loading = false;
		...
		setremarkconfirm: function (done, remark) {
			...
			var that = this
			app.post('ApiAdminHotelOrder/setremark', { orderid: that.detail.id,content:remark }, function (res) {
				app.success(res.msg);
				setTimeout(function () {
					that.getdata();
				}, 1000)
