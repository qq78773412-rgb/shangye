...
<view class="container">
	<block v-if="isload">
		<dd-tab :itemdata="['全部','待审核','已同意','驳回']" :itemst="['all','1','2','-1']" :st="st" :isfixed="true" @changetab="changetab"></dd-tab>
		...
		<view class="order-content">
			<block v-for="(item, index) in datalist" :key="index">
				<view class="order-box" @tap="goto" :data-url="'refundyajinDetail?id=' + item.id">
					<view class="head">
					...
					<view class="content" >
						...
						<view class="detail">
							...
							<text class="t2">{{item.hotelname}}</text>
							<view class="t3">押金：<text class="x1 flex1">￥{{item.refund_money}}</text></view>
						</view>
					</view>
					<view class="op">
						<view class="bottom1">
							<text v-if="item.refund_status==0" style="color:grey"> 申请退押金￥{{item.refund_money}}</text>
							<text v-if="item.refund_status==1" style="color:red"> 退押金中￥{{item.refund_money}}</text>
							<text v-if="item.refund_status==2" style="color:red"> 已退押金￥{{item.refund_money}}</text>
							<text v-if="item.refund_status==-1" style="color:red"> 押金申请已驳回</text>
						</view>
						<view v-if="item.refund_status==1" @tap.stop="refundnopassShow" :data-id='item.id' class="btn2">驳回</view>
						<view v-if="item.refund_status==1" @tap.stop="refundpass" :data-id="item.id" class="btn2">通过</view>
					</view>
			...
			<uni-popup id="dialogSetremark" ref="dialogSetremark" type="dialog">
				<uni-popup-dialog mode="input" title="确定要驳回申请吗？" :value="remark" placeholder="请输入备注" @confirm="refundnopass"></uni-popup-dialog>
			</uni-popup>
		</view>
  ...
  methods: {
    getdata: function (loadmore, keyword) {
      ...
      var st = that.st;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiAdminHotelOrder/refundyajinOrder', {keyword:that.keyword,st: st,pagenum: pagenum,orderid:that.opt.orderid}, function (res) {
				that.loading = false;
    ...
    changetab: function (st) {
      ...
      this.getdata(false);
		...
		refundnopassShow: function (e) {
			var that = this;
			that.orderid = e.currentTarget.dataset.id;
		...
		refundnopass: function (done, remark) {
			...
			app.showLoading();
			app.post('ApiAdminHotelOrder/refundYajinnopass', { orderid: orderid,remark:remark}, function (data) {
		...
		refundpass: function (e) {
			...
			app.confirm(title ? title : '确定要审核通过并退款吗?', function () {
				app.showLoading('提交中');
				app.post('ApiAdminHotelOrder/refundYajinpass', { orderid: orderid }, function (data) {
					app.showLoading(false);
					app.success(data.msg);
					setTimeout(function () {
						that.getdata();
