...
<view class="container">
	<block v-if="isload">
		<dd-tab :itemdata="['全部','待付款','已付款','进行中','已完成','已取消']" :itemst="['all','0','1','2','3','4']" :st="st" :isfixed="true" @changetab="changetab"></dd-tab>
			...
			<block v-for="(item, index) in datalist" :key="index">
				<view class="order-box" @tap.stop="goto" :data-url="'orderdetail?id=' + item.id">
					<view class="head">
						...
						<text v-if="item.status==2" class="st0">进行中</text>
					...
					<view class="content" style="border-bottom:none">
						...
						<view class="detail">
							...
							<text class="t2" v-if="item.cid ==1 || item.cid ==3">
								<text >购买时长</text> ：x {{item.num}}小时</text>
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      var st = that.st;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiCerberuse/orderlist', {st: st,pagenum: pagenum,keyword:that.keyword}, function (res) {
				that.loading = false;
...
.order-box .content .detail .t2{height: 36rpx;line-height: 45rpx;color: #666;overflow: hidden;font-size: 24rpx;}
.order-box .content .detail .t3{display:flex;height:40rpx;line-height:80rpx;color: #ff4246;}
