...
<view class="container">
	<view v-for="(item, index) in datalist" :key="index" class="item">
		<view class="product-item2" @tap="goto" :data-url="'/pagesExt/mingpian/index?id='+item.id">
			<view class="data" :style="{'background':item.bgpic ? 'url('+item.bgpic+')' : '#fff','background-size':'100% 100%'}">
				<view class="data_info" :style="field_list2?'':'border:0;margin:0'">
					<img class="data_head" :src="item.headimg" alt=""/>
					<view>
						<view class="data_name">{{item.realname}}</view>
						<view class="data_text" v-if="item.touxian1">{{item.touxian1}}</view>
						<view class="data_text" v-if="item.touxian2">{{item.touxian2}}</view>
						<view class="data_text" v-if="item.touxian3">{{item.touxian3}}</view>
					</view>
				</view>
				<view class="data_list" v-for="(item2,index2) in field_list2">
					<img v-if="index2 == 'tel'" :src="pre_url+'/static/images/tel.png'" alt=""/>
					<img v-else-if="index2 == 'weixin'" :src="pre_url+'/static/images/weixin.png'" alt=""/>
					<img v-else-if="index2 == 'address'" :src="pre_url+'/static/images/address.png'" alt=""/>
  ...
  methods: {
    getdata: function () {
      ...
      var pagenum = that.pagenum;
			...
			that.nomore = false;
			that.nodata = false;
      app.post('ApiMingpian/mingpianlist', {pagenum: pagenum}, function (res) {
				that.loading = false;
        that.mingpiantext = that.t('名片');
        uni.setNavigationBarTitle({
        	title: that.mingpiantext+'列表'
        });
        var data = res.data;
        if (pagenum == 1) {
					that.datalist = data;
