...
<view class="container">
	<block v-if="isload">
		<view class="ordertop" :style="'background:url(' + pre_url + '/static/img/ordertop.png);background-size:100%'">
			<view class="f1" v-if="detail.l_status==0">
				<view class="t1">待审核</view>
			</view>
			<view class="f1" v-if="detail.l_status==1">
        <view class="t1">已确认收款</view>
			</view>
			<view class="f1" v-if="detail.l_status==2">
				<view class="t1">订单已取消</view>
<!--				<text class="t2" v-if="detail.product_thali_deliver_type==1" user-select="true" selectable="true">发货信息：{{detail.express_com}} {{detail.express_no}}</text>-->
			</view>
      <view class="f1" v-if="detail.l_status==3">
        <view class="t1">已转给上级</view>
      </view>
		</view>
		<view class="address">
			<view class="img">
		...
		<view class="product" v-for="(item, idx) in prolist" :key="idx">
			<view class="content">
				<view @tap="goto" :data-url="'product?id=' + item.proid">
					<image :src="item.pic"></image>
				</view>
				<view class="detail">
					<text class="t1" style="height: 70rpx;overflow: hidden;text-overflow: ellipsis;">{{item.name}}</text>
					<view class="t2 flex flex-y-center flex-bt">
						<text></text>
<!--						<text>{{item.ggname}}</text>-->
						<view class="btn3" v-if="detail.status==3 && item.iscomment==0" @tap.stop="goto" :data-url="'comment?ogid=' + item.id">去评价</view>
      ...
      <view class="item">
        <text class="t1">收款金额</text>
        <text class="t2 red">¥{{detail.money}}</text>
      </view>
      <view class="item">
        <text class="t1">提交审核金额</text>
        <text class="t2 red">¥{{detail.tj_totalprice}}</text>
      </view>
      ...
      <view class="item">
        ...
        <text class="t2" v-if="detail.l_status==0">待审核</text>
        <text class="t2" v-if="detail.l_status==1">已收款</text>
        <text class="t2" v-if="detail.l_status==2">已取消</text>
        <text class="t2" v-if="detail.l_status==3">已转给上级</text>
      </view>
		</view>
      ...
      <block v-if="detail.l_status==1 && detail.status!=4">
        <view class="btn2" @tap="todoorder" :data-id="detail.id" :data-type="3" style="width: 200rpx">转给上级审核</view>
      </block>
			<block v-if="detail.l_status==0 && detail.status!=4">
				<view class="btn2" @tap="todoorder" :data-id="detail.id" :data-type="1">确认收款</view>
				<view class="btn2" @tap="todoorder" :data-id="detail.id" :data-type="2">取消订单</view>
			</block>
...
export default {
  ...
  methods: {
		getdata: function (option) {
			var that = this;
			that.loading = true;
			app.get('ApiTransferOrderParentCheck/orderdetail', {id: that.opt.id}, function (res) {
				that.loading = false;
        if(res.status == 1){
