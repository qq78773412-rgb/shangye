	...
	<block v-if="isload">
		<view class="ordertop" :style="'background:url(' + pre_url + '/static/img/ordertop.png);background-size:100%'">
			...
			<view class="f1" v-if="detail.status==2">
				...
				<text class="t2" v-if="detail.product_thali_deliver_type==0" user-select="true" selectable="true">发货信息：到校领取</text>
			</view>
		...
		<view class="product" v-for="(item, idx) in prolist" :key="idx">
			<view class="content">
				<view @tap="goto" :data-url="'product?id=' + item.gg_proid">
...
<!--						<text>{{item.ggname}}</text>-->
		...
		<view class="bottom notabbarbot" v-if="detail.status!=3">
			...
			<block v-if="(detail.status==2 || detail.status==3) && detail.product_thali_deliver_type==1">
			...
			<block v-if="([1,2,3]).includes(detail.status) && invoice ">
				<!-- <view class="btn2" @tap="goto" :data-url="'/pagesExt/order/invoice?type=collage&orderid=' + detail.id">发票</view> -->
			</block>
			<block v-if="(detail.buytype==1 ) && (detail.status==1 || detail.status==2) && detail.freight_type==1">
...
export default {
  ...
  methods: {
		getdata: function (option) {
			var that = this;
			that.loading = true;
			app.get('ApiProductThali/orderdetail', {id: that.opt.id}, function (res) {
				that.loading = false;
        if(res.status == 1){
