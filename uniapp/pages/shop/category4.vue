			...
			<view class="content" style="height:calc(100% - 94rpx);overflow:hidden;display:flex">
				...
				<view class="nav_right">
					<view class="nav_right-content">
						<scroll-view class="detail-list" :scrollWithAnimation="animation" scroll-y="true" :class="menuindex>-1?'tabbarbot':'notabbarbot'">
							<view v-if="data[currentActiveIndex].banner">
								<image :src="data[currentActiveIndex].banner" style="width:100%;height:auto;margin-bottom:40rpx" mode="widthFix" @tap="gotoCatproductPage" :data-id="data[currentActiveIndex].id"/>
							</view>
							<view class="head" v-else>
								<view class="txt">{{data[currentActiveIndex].name}}</view>
								<view class="show-all" @tap="gotoCatproductPage" :data-id="data[currentActiveIndex].id">查看全部<text class="iconfont iconjiantou"></text></view>
							</view>
							<view class="detail">
								<view v-for="(item, itemIndex) in data[currentActiveIndex].child" :key="itemIndex" @tap.stop="gotoCatproductPage" class="detail-item" :data-id="item.id" form-type="submit" :style="(itemIndex+1)%3===0?'margin-right: 0':''">
									<image class="img" :src="item.pic"></image>
									<view class="txt">{{item.name}}</view>
								</view>
							</view>
							<nodata v-if="(data[currentActiveIndex].child).length == 0"></nodata>
						</scroll-view>
					</view>
				</view>
			</view>
...
.detail-list .head {height: 82rpx;width: 100%;display: flex;align-items:center;justify-content:space-between;margin-bottom:40rpx}
.detail-list .head .txt {color:#222222;font-weight:bold;font-size:28rpx;}
.detail-list .head .show-all {font-size: 22rpx;color:#949494;display:flex;align-items:center}
.detail-list .detail {width:100%;display:flex;flex-wrap:wrap}
.detail-list .detail .detail-item {width:160rpx;height: 160rpx;margin-bottom:70rpx;}
.detail-list .detail .detail-item .img {width: 112rpx;height: 112rpx;margin-left:24rpx}
.detail-list .detail .detail-item .txt {color:#333;font-size: 28rpx;margin-top:20rpx;text-align: center;white-space: nowrap;word-break: break-all;overflow: hidden;display: -webkit-box;-webkit-line-clamp: 1;-webkit-box-orient: vertical;}
