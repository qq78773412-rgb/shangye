			...
			<view class="content" style="height:calc(100% - 94rpx);overflow:hidden;display:flex">
				...
				<view class="nav_right">
					<view class="nav_right-content">
						<scroll-view @scroll="scroll" class="detail-list"  :scrollWithAnimation="animation" scroll-y="true" :class="menuindex>-1?'tabbarbot':'notabbarbot'">
							<view v-for="(detail, index) in clist" :key="index" class="classification-detail-item">
								...
								<view class="detail" v-if="detail.ishide!=1">
									<view v-for="(item, itemIndex) in detail.child" :key="itemIndex" @tap.stop="goto" :data-url="'/activity/kecheng/list?cid='+item.id" class="detail-item" :data-id="item.id" :style="(itemIndex+1)%3===0?'margin-right: 0':''">
...
export default {
  ...
  methods: {
		getdata: function () {
			var that = this;
			that.loading = true;
			app.get('ApiKecheng/category3', {bid:that.bid}, function (res) {
				that.loading = false;
				that.data = res.data;
				that.loaded();
    ...
    getdownclist3: function (id) {
      var that = this;
			that.loading = true;
			that.nodata = false;
      app.post("ApiKecheng/getdownclist3", {id: id,bid:that.bid}, function (res) {
				that.loading = false;
        that.clist = res.data;
...
.classification-detail-item .detail .detail-item {width:160rpx;height: 160rpx;margin-bottom: 36rpx;}
.classification-detail-item .detail .detail-item .img {width: 112rpx;height: 112rpx;margin-left:24rpx}
.classification-detail-item .detail .detail-item .txt {color:#5A5A5A;font-size: 26rpx;margin-top:20rpx;text-align: center;white-space: nowrap;word-break: break-all;overflow: hidden;display: -webkit-box;-webkit-line-clamp: 1;-webkit-box-orient: vertical;}
