	...
	<block v-if="isload">
				...
				<view class="filter-scroll-view">
					<scroll-view class="filter-scroll-view-box" scroll-y="true">
						<view class="search-filter">
							...
							<block v-if="clist2.length > 0">
							<view class="filter-content-title">下级分类</view>
							<view class="search-filter-content">
								<view class="filter-item" :style="catchecid2==''?'color:'+t('color1')+';background:rgba('+t('color1rgb')+',0.1)':''" @tap.stop="cate2Click" data-cid2="">全部</view>
		...
		<view class="product-container">
			<block v-if="datalist && datalist.length>0">
				<dp-kecheng-item v-if="productlisttype=='item2'" :data="datalist" :sysset="sysset" :menuindex="menuindex"></dp-kecheng-item>
				<dp-kecheng-itemlist v-if="productlisttype=='itemlist'" :data="datalist" :sysset="sysset" :menuindex="menuindex"></dp-kecheng-itemlist>
			</block>
...
export default {
  ...
  onLoad: function (opt) {
    ...
    this.chaptertype = this.opt.chaptertype || 0;
		if(this.cpid > 0){
  ...
  methods: {
    ...
    getprolist: function () {
      ...
      that.nomore = false;
			var bid = that.opt.bid ? that.opt.bid : '';
      app.post('ApiKecheng/getlist',{pagenum: pagenum,keyword: keyword,field: field,order: order,gid: gid,cid: cid,cpid:cpid,bid:bid,chaptertype:that.chaptertype}, function (res) {
				that.loading = false;
        var data = res.data;
        if (pagenum == 1) {
          that.datalist = data;
      ...
      that.catchecid = cid;
			if(cid == ''){
			  that.clist2 = [];
				...
				app.get('ApiKecheng/list', {cid:cid}, function (res) {
					that.clist2 = res.clist;
				});
			}
    },
