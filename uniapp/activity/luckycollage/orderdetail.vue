...
<view class="container">
	<block v-if="isload">
		<view class="ordertop" :style="'background:url(' + pre_url + '/static/img/ordertop.png);background-size:100%'">
			...
			<view class="f1" v-if="detail.status==1">
				...
				<block v-if="detail.buytype!=1">
					...
					<block v-else>
						<view class="t2" v-if="team.status==2">拼团成功</view>
					</block>
		...
		<view class="orderinfo">
			...
			<view class="item" v-if="detail.is_score_pay==1 && detail.totalscore>0">
				<text class="t1">{{t('积分')}}支付</text>
				<text class="t2 red">{{detail.totalscore}}</text>
			</view>
			...
			<view class="item">
				...
				<block v-if="detail.status==1">
					<block v-if="detail.buytype!=1">
						...
						<block v-else>
							<text v-if="team.status==2" class="t2">拼团成功,请查看</text>
						</block>
		...
		<view class="bottom notabbarbot" v-if="detail.status!=3">
			...
			<block v-if="([1,2,3]).includes(detail.status) && invoice && team.status==2">
				<view class="btn2" @tap="goto" :data-url="'/pagesExt/order/invoice?type=lucky_collage&orderid=' + detail.id">发票</view>
			</block>
