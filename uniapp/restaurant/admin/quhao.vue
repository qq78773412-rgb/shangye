	...
	<block v-if="isload">
		...
		<view class="form">
			<view class="form-item">
				...
				<input type="text" class="input" placeholder="请输入联系人" name="linkman" :value="linkman" placeholder-style="color:#BBBBBB;font-size:28rpx" @input="inputLinkman"/>
			</view>
			<view class="form-item">
...
export default {
  ...
  methods: {
		getdata: function () {
			var that = this;
			that.loading = true;
			app.get('ApiAdminRestaurantQueue/quhao', {bid:that.opt.bid}, function (res) {
				that.loading = false;
				if(res.status==0){
		...
		confirmQuhao:function(){
			...
			app.showLoading('提交中');
			app.post('ApiAdminRestaurantQueue/quhao',{bid:that.opt.bid,linkman:linkman,tel:tel,renshu:renshu,cid:cid},function(res){
