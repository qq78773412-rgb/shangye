...
<view class="container">
	<block v-if="isload">
		...
		<view class="product">
			<view v-for="(item, idx) in prolist" :key="idx" class="content" v-if="shopset.is_times_prolist ==0">
			...
			<!-- 带下单次数的列表 -->
			<view class="timeslist" v-for="(times, index) in prolist" :key="idx" v-if="shopset.is_times_prolist ==1">
		...
		<view class="bottom notabbarbot">
			<block v-if="detail.status==0">
				...
				<view class="btn1" :style="{background:t('color1')}" @tap="toPay" :data-id="detail.payorderid" :data-istopay="detail.is_topay" >去付款</view>
			</block>
