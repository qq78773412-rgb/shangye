...
export default {
  ...
  onLoad: function (opt) {
    ...
    if (productlisttype) this.productlisttype = productlisttype;
		this.history_list = app.getCache('search_history_list_rest_takeaway');
		this.getdata();
  },
  ...
  methods: {
    ...
    getprolist: function () {
      ...
      that.nomore = false;
			var bid = that.opt.bid ? that.opt.bid : '';
      app.post('ApiRestaurantTakeaway/getprolist',{pagenum: pagenum,keyword: keyword,field: field,order: order,gid: gid,cid: cid,cpid:cpid,bid:bid}, function (res) {
				that.loading = false;
        var data = res.data;
        if (pagenum == 1) {
          that.datalist = data;
    ...
    addHistory: function () {
      ...
      var historylist = app.getCache('search_history_list_rest_takeaway');
      ...
      app.setCache('search_history_list_rest_takeaway', newhistorylist);
    ...
    deleteSearchHistory: function () {
      ...
      app.removeCache("search_history_list_rest_takeaway");
    },
		...
		addcart:function(e){
			...
			that.loading = true;
			app.post('ApiRestaurantTakeaway/addcart', {proid: proid,ggid: ggid,num: num,bid:that.opt.bid}, function (res) {
				that.loading = false;
				if (res.status == 1) {
