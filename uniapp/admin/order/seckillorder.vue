	...
	<block v-if="isload">
		...
		<view class="order-content">
			<block v-for="(item, index) in datalist" :key="index">
				<view class="order-box" @tap="goto" :data-url="'seckillorderdetail?id=' + item.id">
					<view class="head">
  ...
  methods: {
    getdata: function (loadmore,keyword) {
      ...
      var st = that.st;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiAdminOrder/seckillorder', {keyword:that.keyword,st: st,pagenum: pagenum}, function (res) {
				that.loading = false;
