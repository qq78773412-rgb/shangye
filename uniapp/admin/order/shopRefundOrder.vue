...
<view class="container">
	<block v-if="isload">
		<dd-tab :itemdata="['全部','待审核','已同意','驳回']" :itemst="['all','1','2','3']" :st="st" :isfixed="true" @changetab="changetab"></dd-tab>
  ...
  methods: {
    getdata: function (loadmore, keyword) {
      ...
      var st = that.st;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiAdminOrder/shopRefundOrder', {keyword:that.keyword,st: st,pagenum: pagenum,orderid:that.opt.orderid}, function (res) {
				that.loading = false;
