	...
	<block v-if="isload">
		...
		<view class="order-content">
			<block v-for="(item, index) in datalist" :key="index">
				<view class="order-box" @tap="goto" :data-url="'kanjiaorderdetail?id=' + item.id">
					<view class="head">
					...
					<view class="content" style="border-bottom:none">
						...
						<view class="detail">
							...
							<view class="t3"><text class="x1 flex1">￥{{item.sell_price}}</text><text class="x2">×1</text></view>
						</view>
					</view>
					<view class="bottom">
						<text>共计1件商品 实付:￥{{item.totalprice}}</text>
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      var st = that.st;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiAdminOrder/kanjiaorder', {keyword:that.keyword,st: st,pagenum: pagenum}, function (res) {
				that.loading = false;
