...
<view class="container">
	<block v-if="isload">
		<dd-tab v-if="!goods_hexiao_status" :itemdata="['全部','待付款','待发货','待收货','已完成','退款']" :itemst="['all','0','1','2','3','10']" :st="st" :isfixed="true" @changetab="changetab"></dd-tab>
        <dd-tab v-else :itemdata="['全部','待付款','待发货','已接单','待收货','已完成','退款']" :itemst="['all','0','1','12','2','3','10']" :st="st" :isfixed="true" @changetab="changetab"></dd-tab>
		...
		<view class="order-content">
			<block v-for="(item, index) in datalist" :key="index">
				<view class="order-box" @tap="goto" :data-url="'takeawayorderdetail?id=' + item.id">
					<view class="head">
						...
						<view class="content" :style="idx+1==item.procount?'border-bottom:none':''">
							...
							<view class="detail">
								...
								<view class="t3">
                                    ...
                                    <text v-if="goods_hexiao_status && (item.status==1 || item.status==2 || item.status==12) && item.freight_type==1 && item2.hexiao_code && item2.num>0 " style="color: #999;">
                                        <block v-if="item2.status == 3" style="color: #999;">已核销</block>
                                        <block v-else style="color: #f60;">未核销</block>
                                    </text>
					...
					<view class="bottom flex-y-center">
						<image :src="item.member.headimg" style="width:40rpx;height:40rpx;border-radius:50%;margin-right:10rpx"/>
                        <text style="font-weight:bold;color:#333;margin-right:8rpx">{{item.member.nickname}}</text>
                        (ID:{{item.mid}})
					</view>
                    <view v-if="goods_hexiao_status &&(item.status == 12 || item.status == 2 )&& item.tel" @tap.stop="callphone" :data-phone="item.tel" style="width: 150rpx;text-align: center;line-height: 60rpx;border-radius: 8rpx;border: 2rpx solid #333;">
                        拨打电话
                    </view>
				</view>
			</block>
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      var st = that.st;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiAdminRestaurantTakeawayOrder/index', {keyword:that.keyword,st: st,pagenum: pagenum}, function (res) {
			that.loading = false;
