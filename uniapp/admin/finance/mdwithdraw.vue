...
<view class="container">
	...
	<form @submit="formSubmit">
		<view class="mymoney" :style="{background:t('color1')}">
			...
			<view class="f3" @tap="goto" data-url="mdwithdrawlog"><text>提现记录</text><text class="iconfont iconjiantou" style="font-size:20rpx"></text></view>
		</view>
		<view class="content2">
  ...
  <view class="op" v-if="sysset.mendian_money_transfer">
    <view class="btn" @tap="goto" data-url="/adminExt/finance/transfermendianmoney" :style="{background:t('color2')}">转账</view>
  </view>
	</block>
...
export default {
  ...
  methods: {
		getdata: function () {
			var that = this;
			that.loading = true;
			app.get('ApiAdminFinance/mdwithdraw', {}, function (res) {
				that.loading = false;
    ...
    formSubmit: function (e) {
      ...
      if (paytype == '银行卡' && (!this.userinfo.bankname || !this.userinfo.bankcarduser || !this.userinfo.bankcardnum)) {
        ...
        return;
			...
			app.showLoading('提交中');
      app.post('ApiAdminFinance/mdwithdraw', {money: money,paytype: paytype}, function (res) {
...
.op .btn{flex:1;height:100rpx;line-height:100rpx;background:#07C160;width:90%;margin:0 20rpx;border-radius:50rpx;color: #fff;font-size:28rpx;font-weight:bold;display:flex;align-items:center;justify-content:center}
