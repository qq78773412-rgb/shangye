...
<view>
	<block v-if="isload">
		...
		<view class="content" v-if="datalist && datalist.length>0">
			<view class="label">
				<text class="t1">{{t('佣金')}}明细（共{{count}}条）</text>
			</view>
			<view v-for="(item, index) in datalist" :key="index" class="item">
				...
				<view class="f2">
					<text class="t1" v-if="item.commission>0" style="color:#000">+{{item.commission}}元</text>
					<text class="t1" v-else>{{item.commission}}元</text>
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      var keyword = that.keyword;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiAdminFinance/commissionlog', {keyword:keyword,pagenum: pagenum}, function (res) {
				that.loading = false;
        var data = res.data;
        if (pagenum == 1){
