	...
	<block v-if="isload">
		...
		<view class="product" v-for="(item, idx) in prolist" :key="idx">
			<view class="content">
				...
				<view class="detail">
					<text class="t1">{{item.gg_proname}}</text>
		...
		<view class="bottom notabbarbot" v-if="detail.status!=3">
			...
			<block v-if="detail.status==1">
				<view v-if="(detail.refund_status==0 || detail.refund_status==3) && shopset.refund==1" class="btn2" @tap="goto" :data-url="'refund?orderid=' + detail.id + '&price=' + detail.totalprice">申请退款</view>
			</block>
...
export default {
  ...
  methods: {
		getdata: function (option) {
			var that = this;
			that.loading = true;
			app.get('ApiTaocan/orderdetail', {id: that.opt.id}, function (res) {
				that.loading = false;
        if(res.status == 1){
