...
<view class="container">
	<block v-if="isload">
		<view class="myscore" :style="{background:t('color1')}">
			<view class="f1">我的{{t('分红份数')}}</view>
			<view class="f2">{{mycopies}}</view>
		</view>
		<view class="content" id="datalist">
			...
			<view class="item">
				...
				<view class="f2">
					<block v-if="item.copies>0">
						<text class="t1">+{{item.copies}}</text>
					</block>
					<block v-else>
						<text class="t2">{{item.copies}}</text>
					</block>
				</view>
			</view>
		</block>
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      var st = that.st;
			...
			that.nomore = false;
			that.loading = true;
      app.get('ApiMy/fhcopieslog', {st: st,pagenum: pagenum}, function (res) {
				that.loading = false;
        var data = res.datalist;
        if (pagenum == 1) {
					that.mycopies = res.mycopies;
					that.datalist = data;
          if (data.length == 0) {
            that.nodata = true;
          }
