...
<view class="container">
	<block v-if="isload">
		<dd-tab :itemdata="[t('贡献值')+'明细','提现记录']" :itemst="['0','1']" :st="st" :isfixed="false" @changetab="changetab"></dd-tab>
		<view class="content">
			<block v-if="st==0">
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      var pagenum = that.pagenum;
			...
			that.nodata = false;
      that.nomore = false;
      app.post('ApiAgent/gongxiancommissionlog', {st: st,pagenum: pagenum,type:type}, function (res) {
				that.loading = false;
        var data = res.data;
        if (pagenum == 1) {
