	...
	<block v-if="isload">
		...
		<view v-for="(item, index) in datalist" :key="index" class="content" @tap.stop="setdefault" :data-id="item.id">
			<view class="f1">
				...
				<image class="t3" :src="pre_url+'/static/img/edit.png'" @tap.stop="goto" :data-url="'caradd?id=' + item.id">
			</view>
			<view class="f2">{{item.number}}.{{item.color}}.{{item.type==1?'SUV':item.type==2?'MPV':'轿车'}}</view>
			<view class="f3">
				<view class="flex-y-center">
					...
					<view class="mrtxt">{{item.isdefault ? '默认车辆' : '设为默认'}}</view>
				</view>
		...
		<view class="btn-add" :class="menuindex>-1?'tabbarbot':'notabbarbot3'" :style="'background:linear-gradient(90deg,'+t('color1')+' 0%,rgba('+t('color1rgb')+',0.8) 100%)'" @tap="goto" :data-url="'caradd'"><image :src="pre_url+'/static/img/add.png'" style="width:28rpx;height:28rpx;margin-right:6rpx"/>添加车辆</view>
	</block>
...
export default {
  ...
  methods: {
		getdata:function(){
			...
			that.loading = true;
			that.nodata = false;
			app.get('ApiYuyuecar/car', {type: that.opt.type,keyword:that.keyword}, function (res) {
				that.loading = false;
				var datalist = res.data;
				if (datalist.length == 0 && that.keyword == ''){
					uni.redirectTo({
						url: '/pagesA/yuyuecar/caradd?type=' + (that.opt.type || 0)
					});
    ...
    setdefault: function (e) {
      ...
      app.post('ApiYuyuecar/setdefault', {carid: carId}, function (data) {
        if (fromPage) {
          app.goback(true);
        } else {
          that.getdata();
    ...
    del: function (e) {
      ...
      var carId = e.currentTarget.dataset.id;
      console.log(carId);
      app.confirm('确定要删除此车辆吗?', function () {
        app.post("ApiYuyuecar/del", {carid: carId}, function (res) {
          app.success(res.msg);
          that.getdata();
        });
      });
