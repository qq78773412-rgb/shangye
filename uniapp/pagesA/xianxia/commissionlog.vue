	...
	<block v-if="isload">
		...
		<view class="content" v-if="datalist && datalist.length>0">
			<block v-for="(item, index) in datalist" :key="index">
				<view class="item">
					<view class="title" style="line-height: 40rpx;">奖励发放</view>
					<view class="f1" @click="toteam(item.id)">
						<image :src="item.headimg"></image>
						<view class="t2" style="flex: 1;">
							<text class="x1">{{item.nickname}}(ID:{{item.tomid}})</text>
							<text class="x1">等&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;级：{{item.levelname}}</text>
							<text class="x1" v-if="item.tel">手&nbsp;&nbsp;机&nbsp;号：{{item.tel}}</text>
							<text class="x1" >产生时间：{{item.createtime}}</text>
					...
					<view class="f2">
						...
						<view class="t1">{{item.remark}}</view> 
						<button v-if="item.status ==0" class="btn" :style="{background:'linear-gradient(270deg,'+t('color1')+' 0%,rgba('+t('color1rgb')+',0.8) 100%)'}" @tap="toSendCommission" :data-id="item.id" >发放奖励</button>
						<button v-if="item.status ==1" class="btn" :style="{background:'linear-gradient(270deg,'+t('color1')+' 0%,rgba('+t('color1rgb')+',0.8) 100%)'}" @tap="toSendCommission" :data-id="item.id" >查看详情</button>
					</view>
				</view>
			</block>
		</view>
...
export default {
  ...
  methods: {
    getdata: function (loadmore) {
		...
		that.nomore = false;
		that.nodata = false;
		app.post('ApiCoupon/xianxianCommissionLog', {st: st,pagenum: pagenum,type:'send'}, function (res) {
			that.loading = false;
